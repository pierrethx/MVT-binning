...
sublist=["target3","target5","target10"]
tlist=[3,5,10]
markers=["s","v","*"]
#sublist=["target20","target40","target60"]
#tlist=[20,40,60]
tag="big_"
cvt=False
#sublist=["target20"]
wcsxlist,siglist,varlist,sourcelist,objlist=bin_accretion.minitialize()
namelist=["_".join(obj.split("_")[:-1]) for obj in objlist]
...
for i in range(len(sourcelist)):
    ...
    for sub in sublist:
        ...
        '''
        if cvt:
            with fits.open(sourcelist[i]+"/"+sub+"/"+"z_"+namelist[i]+"_c_assigned.fits") as hdul:
                ass=(np.flipud(hdul[0].data))
            with fits.open(sourcelist[i]+"/"+sub+"/"+"zston_"+namelist[i]+"_c.fits") as hdul:
                bston=(np.flipud(hdul[0].data))
        else:
            with fits.open(sourcelist[i]+"/"+sub+"/"+"z_"+namelist[i]+"_assigned.fits") as hdul:
                ass=(np.flipud(hdul[0].data))
            with fits.open(sourcelist[i]+"/"+sub+"/"+"zston_"+namelist[i]+".fits") as hdul:
                bston=(np.flipud(hdul[0].data))
    ...
    ax.plot(unrad[unrad>60],unsn[unrad>60],linewidth=0,marker=".",label="unbinned",zorder=0)
    for intt in range(len(sublist)):
        ax.plot(np.linspace(60,maxx*1.1,10),0*np.linspace(60,maxx*1.1,10)+tlist[intt],linewidth=1,color="k",linestyle="dashed",zorder=5)
    ...
    #plt.show()
    fig.savefig(sourcelist[i]+"/"+tag+namelist[i]+"_SNR.png")
    fig2.savefig(sourcelist[i]+"/"+tag+namelist[i]+"_round.png")
    ...
    fig3.legend()
    fig3.savefig(sourcelist[i]+"/"+tag+namelist[i]+"_areas.png")
