...
for beta in ["beta0.67","beta1.17","beta1.67","beta2.17","beta2.67","beta3.17"]:
    ...
    for q in range(len(param_levels)):
        with fits.open(home_directory+"/"+beta+"/unbinned/testdata_"+param_levels[q]+"_model.fits") as hdul:
            model=hdul[0].data

        mask=np.full_like(model,1)
        with fits.open(home_directory+beta+"/unbinned//testdata0_"+param_levels[q]+"_signal.fits") as hdul:
  ...
  #    unbinnedvar_m[n]=np.sum((hdul[0].data-model)**2)
        for targi in range(len(targ_levels)):
            with fits.open(home_directory+beta+"/"+targ_levels[targi]+"/block_testdata0_"+param_levels[q]+"_wit_sig.fits") as hdul:
    ...
    #plt.legend()
    #plt.show()
    plt.savefig(home_directory+"/model_recovery_"+beta+".png",bbox_inches="tight")