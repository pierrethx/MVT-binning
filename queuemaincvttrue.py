...
def mainfunc(signal,var,target,displayWVT=False,epsilon=10):
    ...
    binlist,diflist=iteration_func(target,signal,var,init_generators,epsilon,displaywvt=False)
...
if __name__ == "__main__":
    ...
    for i in range(len(sourcelist)):
        for m in range(len(targlist)):
            ...
            weighting=signal2/var2
            weighting = np.where(np.isnan(weighting),0,weighting)
            print(np.nanmax(weighting))
            print(np.nanmin(weighting))
            print(np.nanmedian(weighting))
            print(np.nanmean(weighting))
            weighting=weighting/np.nanmedian(weighting)
            signal2=signal2*weighting
            var2=var2*weighting
            
            raise NameError("boog")
            signal2,var2=makecopy(signal2,var2)
            binlist,diflist=mainfunc(signal2,var2,target,displayWVT=False,epsilon=eps)
            ...
            main.saveiteratedfits(target,wcsx,wvt,vwvt,objname,sourcedir,subfolder=subfolder,weighting=False)
            functions.convergence(eps,diflist,sourcedir,objname,subfolder=subfolder)
            main.saveblockoutfits(target,ston,wcsx,wvt,vwvt,objname,sourcedir,subfolder=subfolder,weighting=False)
